...
Since a planned refactoring will eliminate all of the global variables and most, if not all, `static` methods, this approach will allow independently allocated parser objects to maintain their own current scopes separate from each other.
...
The two changes I did make to `TypeInfo` were to redefine its `fields` property as an array of `SymbolType` rather than a linked list as it was in the original, and to redefine the `form` property to be an `enum Form` type.  As was the case for using an `enum` for `SymbolInfo.kind`, this helps with type-checking, since it is not interchangeably with any other type.
